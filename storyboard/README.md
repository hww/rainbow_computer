...
Усилитель собран на половине 548УН1А,а выходной каскад на КТ814,КТ815. Большей части дискретных эелементов усилителя на плате нет. Поэтому я нарисую заново схему усилителя. 

На воторой половинке собран генератор шума для звуковой системы. Возможно испольуется КТ3102 с базой на общем, коллектором не подключенным и эмитером кудато в операционнике.

### 02.04.2019 УСИЛИТЕЛЬ МАГНИТОФОНА

На этой плате он собран на 140УД6. Схема напоминает ту что в СПЕЦИАЛИСТе, но есть и отличия. Однако опять, элементов в плате недостатоно чтобы повторить этот вариант. Я буду использовать вариант из СПЕЦАЛИСТа. Выход схемы подключен к PPI1.PB7
...
После небольшой паузиы частичто срисован разъем расширения. Судя по всему кроме клавиатуры на разъем выведен парралельный интерфейс. Возможно принтер или универсальный порт расширения.

![Expansion 1](./images/expansion_1.png)

### 11.04.2019 Системный Разъем 

Два 8ми битных порта PPI2 подключены к разьему расширения. Это позволяет подключать различные перифериные устройства с прграммируемым интерфейсом: принтер, джойстик, covox, контроллер FDC

![Expansion 2](./images/expansion_2.png)

Таким образом функции PPI2 можно считать польностью понятными. Исключение в том что не ясно как они использовались перифериным устройстом или внутри корпуса компьютера. Возможно что на некоторых пинах был переключатель с задней панели, а возможно что на некоторых был подключен светодиод.

![PPI2](./images/ppi2_1.png)

Функции PPI1 можно считать почти польностью понятными. Исключение PPI1.PB6, его назначение пока не известно.

![PPI1](./images/ppi1_1.png)

*Небольшое дополнение:* _Подключение выводов PPI1 и PPI2 к системному разьему не дает полной информации об их использовании внутри платы. Дальнейшее исследование может показать что некоторые из этих выводов имеют какуюлибо функцию внутри платы._ 
...
Считывание данных показало другой резльтат (файл тут [ROM](../rom/rainbow-font.BIN)) 

Фонт выглядит так как на следующем рисунке.

![Raibow Font PNG](../rom/rainbow-font-image.png)

Размер символа 5x7 поэтому теоретически для монохромного экрана позволял отображать 64 символа в строке. К сожалению заветных 80-ти символов небыло, но и 64 лучше чем 48.

Невозможно утверждать был ли этот фонт конечный, или использовался только в прототипе. Но даже если фонт изменился и стал 7х7 врядли изменилась кодировка. А кодировка должна соотвествовать кодировке в MSX.

### 13.04.2019 Бракованые DRAM 

В прототипе стояд микросхемы памяти с браком, старшая линия ROWS отсутсвует. Действительно, я помню что приходили и использовались микросхемы у которых было только половина памяти.

### 14.04.2019 Короткий отчет по прогессу
...
Вот тк может выглядеть логика работы этой схемы.

![COPIER 2](./images/copier-2.png)

Иными словами можно читать и писатьпо адресу копира FB00. 
Алтернативно можно считывая днные из страницы 2 (8000-BFFF) модифицировать "копир" цветом. Записывая в страницу 2 модифировать буфер цвета содержимым копира.
